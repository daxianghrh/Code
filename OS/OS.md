...
-  `brk`系统调用从堆分配内存：[堆顶]指针向高地址移动。
-  `mmap`系统调用在文件映射区域分配内存：在文件映射区分配一块内存

注：`malloc`存在内存分配阈值，当内存小于`128KB`，通过`brk`申请内存；大于`128K`时，通过`mmap`申请内存。
...
TCP（传输控制协议）是一种面向连接的协议，用于在计算机之间可靠地传输数据。在TCP通信过程中，数据被分成小的单元，然后在发送端和接收端之间传输。两个相关的概念是TCP的分段（Segmentation）和IP层的分片（Fragmentation），它们用于处理不同层次的数据分割和重组。

1. **TCP的分段（Segmentation）**：
   - TCP将应用层数据（例如，HTTP请求或文件内容）分割成更小的单元，这些单元称为TCP段或TCP分段。每个TCP段都有一个序列号（Sequence Number），用于在接收端对它们进行排序和重组。
   - 分段的主要目的是将大的数据流分解成小的块，以便更容易处理和传输。这有助于防止网络拥塞，因为较小的分段更容易在网络上传输。
   - 发送方将分段发送到接收方，接收方根据序列号对它们进行排序并重组，以还原原始数据流。
   - 分段通常包括TCP头部，其中包含源端口、目标端口、序列号、确认号等信息，以及TCP数据。TCP头部占用一定的字节，因此分段的大小不仅受应用程序数据大小的限制，还受TCP头部的大小影响。

2. **IP层的分片（Fragmentation）**：
   - IP层负责将数据报从源主机传输到目标主机，而不管数据报的大小。然而，网络链路和设备可能会限制传输的数据包大小。当数据报的大小超过某个网络链路或设备的最大传输单元（Maximum Transmission Unit，MTU）时，IP层需要将数据报分割成更小的单元，这个过程称为分片。
   - 分片后的数据包会在网络上单独传输，然后在接收端重新组装成原始数据报。
   - 分片的主要目的是确保数据可以在网络上传输，尽管不同链路和设备可能有不同的MTU限制。
   - 分片是由IP层处理的，而不涉及TCP层。TCP只关心TCP分段的大小和顺序。

总的来说，TCP的分段是为了将应用层数据分割成可管理的单元，以便在网络上传输，并在接收端重新组装。而IP层的分片是为了适应不同网络链路和设备的MTU限制，以确保数据能够在网络上传输。这两个过程共同确保数据能够可靠地传输到目标主机。需要注意的是，过多的分段和分片可能会导致性能下降，因此需要谨慎处理。
